...
class CrystalGraphConvNetMulti(nn.Module):
    """
    ...
    def __init__(self, orig_atom_fea_len, nbr_fea_len,
        ...
        n_h: int
          Number of hidden layers after pooling
        """
        super(CrystalGraphConvNetMulti, self).__init__()
        self.embedding = nn.Linear(orig_atom_fea_len, atom_fea_len)
        self.convs = nn.ModuleList([ConvLayer(atom_fea_len=atom_fea_len,
                                    nbr_fea_len=nbr_fea_len)
                                    for _ in range(n_conv)])
        ...
        # 回归任务输出层
        self.fc_out_reg = nn.Linear(h_fea_len, 1)
        
        # 分类任务输出层
        self.fc_out_cls = nn.Linear(h_fea_len, 1)
    ...
    def forward(self, atom_fea, nbr_fea, nbr_fea_idx, crystal_atom_idx):
        ...
        # 回归任务输出
        regression_output = self.fc_out_reg(crys_fea)
        
        # 分类任务输出
        classification_output = self.fc_out_cls(crys_fea)
